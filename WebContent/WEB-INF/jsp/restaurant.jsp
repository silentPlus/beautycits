  ...
  <!--<![endif]-->
	    ...
	    <ul>
		    ...
		    <li>
			    <ul class="restaurant-menu nav nav-list collapse in">
		            <li onclick=""><a><span class="fa fa-caret-right"></span>饭店信息管理</a></li>
    ...
    <div class="content">
        ...
        <div class="main-content">
			...
			<div class="btn-toolbar list-toolbar">
				<div>
	            	<table class="table" style="text-align:center;">
	            		<tr>
	            			<td width="10%" align="right" style="border-top:none;">饭店名称:</td>
	            			<td width="40%" align="left" style="border-top:none;">
	            				<select id="restauranttypeid" class="form-control">
	            			...
	            			<td width="40%" style="border-top:none;">
	            				<select id="format" class="form-control">
	            					  <option value="" checked="checked">请选择</option>
						              <option value="0">小桌（1~4人）</option>
						              <option value="1">中桌（5~8人）</option>
						              <option value="2">大桌（>8人）</option>
					        	</select>
	            			</td>
	            		</tr>
	            		<tr>
			    ...
			    <button id = "addRestaurant" class="btn btn-primary"><i class="fa fa-plus"></i>&nbsp;添加饭店信息</button>
		  		<div class="btn-group">
		  		</div>
			</div>
			
			  <div id="restaurantsTable">
			  <script id="restaurantsTemplateView" type="text/html">
			  <table class="table" style="text-align:center;">
			  <thead>
			    <tr>
			      <th style="width:3%;text-align: center;">#</th>
			      <th style="width:10%;text-align: center;">饭店名称</th>
			  ...
			  {{ each restaurants as restaurant i }}
			    <tr>
			      ...
			      <td>{{restaurant.formatname}}</td>
			      <td>{{restaurant.area}}</td>
			      <td>{{restaurant.cost}}元</td>
			      <td>{{restaurant.remark}}</td>
				  <td><a class="deleteModelBtn" restaurantid="{{restaurant.id}}"><i class="fa fa-trash-o"></i></a></td>
			    </tr>
			  {{ /each }}
			  </tbody>
			</table>
			...
			<div class="modal small fade" id="addRestaurantModel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			  <div class="modal-dialog" style="width:800px;">
			    <div class="modal-content">
			        <div class="modal-header">
			            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			            <h3 id="myModalLabel">添加饭店信息</h3>
			        </div>
			        <div class="modal-body">
			            <table class="table" style="text-align:center;">
		            		<tr>
		            			<td align="right" style="border-top:none;">饭店名称</td>
		            			<td align="left" style="border-top:none;">
		            				<select id="rrestauranttypeid" class="form-control">
	            					  <option value="" checked="checked">请选择</option>
           					    </select>
		            			</td>
			        ...
			        <div class="modal-footer">
			            ...
			            <button class="btn btn-danger addRestaurantBtn" data-dismiss="modal">确定</button>
			        </div>
			      </div>
			    </div>
			</div>
			...
			<div class="modal small fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			  <div class="modal-dialog">
			    <div class="modal-content">
			        <div class="modal-header">
			            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			            <h3 id="myModalLabel">删除饭店信息</h3>
			        </div>
			        <div class="modal-body">
			            <p class="error-text"><i class="fa fa-warning modal-icon"></i>确定删除该饭店信息?<br>操作不可恢复。</p>
			        </div>
			        <div class="modal-footer">
			            <button class="btn btn-default" data-dismiss="modal" aria-hidden="true">取消</button>
    ...
    <script type="text/javascript">
	    $(function(){
	    	...
	    	var arrayObj = new Array(restaurantPage.totalPage);
	    	for (var i=0; i<restaurantPage.totalPage; i++){
	    		arrayObj[i] = i;
	    	}
	    	data = {
	    			restaurants : restaurantPage.pageInfoResult,
	    			length : restaurantPage.totalRecord,
	    			currentPage : restaurantPage.currentPage,
	    			totalPage : restaurantPage.totalPage,
	    			list : arrayObj
	    	};
	    	var restaurantsViewHtml = template("restaurantsTemplateView", data);
	    	$("#restaurantsTable").html(restaurantsViewHtml);
	    	
	    	var listType = ${listType};
	    	for (i = 0; i < listType.length; i++) {
	    		$('#restauranttypeid').append('<option value="' + listType[i].id + '">'+ listType[i].name + '</option>');
	    		$('#rrestauranttypeid').append('<option value="' + listType[i].id + '">'+ listType[i].name + '</option>');
    		} 
	    	...
	    	$("#addRestaurant").click(function(){
	    		$("#addRestaurantModel").modal('show');
	    	});
	    	
	    	$(".addRestaurantBtn").on("click", function(){
	    		var restauranttypeid = $.trim($("#rrestauranttypeid").val());
	    		...
	    		if (restauranttypeid == null || restauranttypeid == '') {
	    			alert("请选则饭店类型");
	    			return ;
	    		}
	    		...
	    		$.ajax({
	    			url : "${ctx}/restaurant/addrestaurant.html",
	    			async : false,
	    			type : 'POST',
	    			cache:false,
	    			data : {
    ...
    /*     $(function() {
		...
		$("#deleteBtn").click(function(){
    		...
    		$.ajax({
    			url : "${ctx}/restaurant/deleterestaurant.html",
    			async : false,
    			type : 'POST',
    			cache:false,
    			data : {
	...
	function dosearch(i){
		...
		var restauranttypeid = $.trim($("#restauranttypeid").val());
    	...
    	if (i == -2) {
   		...
   		$.ajax({
   			url : "${ctx}/restaurant/search.html",
   			async : false,
   			type : 'POST',
   			cache:false,
   			data : {
   			...
   			success : function(response) {
   				...
   				if (0===json.status){
   			    	...
   			    	data = {
   			    			restaurants : result.pageInfoResult,
